...
> Everything below this line is local to ECHO 回响 — it's how the tokens above are wired into Tailwind, not part of the original VoltAgent/awesome-design-md source. Source of truth for the tokens themselves is still the YAML above; this section just maps them to class names.

This file was copied verbatim from `awesome-design-md/design-md/apple/DESIGN.md` (VoltAgent/awesome-design-md). To refresh: `cd awesome-design-md && git pull`, then re-copy and re-diff this file and `app/globals.css` by hand — tokens are not synced automatically.

**Where the tokens live:** `app/globals.css`, inside a Tailwind v4 `@theme` block. No `tailwind.config.ts` — v4 reads theme values straight out of CSS custom properties.

**Colors** — same names as the YAML `colors:` keys, as Tailwind color utilities: `bg-primary`, `text-ink`, `bg-canvas-parchment`, `border-hairline`, `bg-surface-tile-1`, etc.

**Typography** — each YAML `typography:` key is a `text-*` utility bundling font-size + line-height + letter-spacing + font-weight (`text-hero-display`, `text-body`, `text-caption`, ...). Tailwind's `text-*` utility has no font-*family* slot, so pair every type token with a family utility by hand:

| Family (per YAML `fontFamily`) | Utility class | Tokens that use it |
|---|---|---|
| SF Pro Display | `font-display` | `hero-display`, `display-lg`, `lead`, `tagline` |
| SF Pro Text | `font-text` | `display-md`, `lead-airy`, `body-strong`, `body`, `dense-link`, `caption`, `caption-strong`, `button-large`, `button-utility`, `fine-print`, `micro-legal`, `nav-link` |

Example: `<h1 className="font-display text-hero-display">`. `display-md` is intentionally paired with `font-text` (not `font-display`) — that's in the source YAML, not a bug (see "Section heads (SF Pro Text at display proportions)" above).

SF Pro itself isn't bundled (Apple proprietary, not redistributable). `font-display`/`font-text` both resolve to `-apple-system, BlinkMacSystemFont` first — real SF Pro on macOS/iOS/Safari — falling back to Inter (loaded via `next/font/google` in `app/layout.tsx` as `--font-inter`) everywhere else, per this doc's own "Note on Font Substitutes" above.

**Spacing** — YAML `spacing:` keys are named Tailwind spacing utilities: `p-lg` (24px), `gap-section` (80px), etc., additive to Tailwind's default numeric scale (`p-4` etc. still work).

> **Gotcha (already hit once, don't reintroduce it):** Tailwind v4's `--spacing-*` theme namespace is shared by *every* sizing utility, not just padding/margin/gap — `max-w-*`, `w-*`, `h-*`, `min-w-*`, etc. all check it too, and `--spacing-<key>` wins over Tailwind's own `--container-<key>` scale when both exist for the same name. Since our spacing scale reuses the names `xs`, `sm`, `md`, `lg`, `xl` — Tailwind's own t-shirt-size vocabulary for `max-w-*` — those five keys are shadowed: **`max-w-sm`/`max-w-md`/`max-w-lg`/`max-w-xl` no longer mean Tailwind's stock 24rem/28rem/32rem/36rem, they mean our 12px/17px/24px/32px spacing values instead**, which silently collapses any element using them to a near-zero-width column (each word wraps onto its own line). `max-w-2xl` and up are unaffected (we didn't define `spacing.2xl`). When you want a genuine prose/content max-width, don't use the bare `max-w-{xs,sm,md,lg,xl}` classes — use `max-w-2xl`+ or an arbitrary value (`max-w-[28rem]`) instead.

**Radius** — YAML `rounded:` keys: `rounded-xs` (5px) ... `rounded-lg` (18px), `rounded-pill` (9999px). `rounded-none` and `rounded-full` are Tailwind's stock utilities and already match the YAML values (0px / 9999px).

**Shadow** — `shadow-product` is the *only* shadow utility defined. Do not add others; see Do's and Don'ts above.

**Motion** — the source doc documents exactly one interaction: `transform: scale(0.95)` on press (`active:scale-95` + a stock `transition-transform duration-150 ease-out` in this project — duration/easing aren't specified in the source, 150ms/ease-out is a conventional default, not an extracted value). Per the doc's own Iteration Guide ("Never document hover. Default and Active/Pressed states only."), components in this project intentionally have **no hover states**, only default and active/press.
